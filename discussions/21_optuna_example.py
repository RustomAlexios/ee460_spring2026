...
def define_model(trial):
    ...
    for i in range(n_layers):
        # Number of units in each hidden layer (hyperparameter)
        out_features = trial.suggest_int("n_units_l{}".format(i), 4, 128)
        layers.append(nn.Linear(in_features, out_features))
        # Activation function
        layers.append(nn.ReLU())
        # Dropout for regularization (hyperparameter)
    ...
    # Return model as a sequential container
...
def objective(trial):
    ...
    # Choose optimizer (categorical hyperparameter)
...
if __name__ == "__main__":
    ...
    # Create Optuna study (maximize accuracy)
    study = optuna.create_study(direction="maximize")
    
    # Run optimization (100 trials/ 10 minutes)
    study.optimize(objective, n_trials=100, timeout=600)

    # Collect statistics about trials
